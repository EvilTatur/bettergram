...
void MainWindow::psFirstShow() {
	...
	if (QDBusInterface("com.canonical.Unity", "/").isValid()) {
		...
		if(snapName.isEmpty()) {
			if(!QStandardPaths::locate(QStandardPaths::ApplicationsLocation, "bettergram.desktop").isEmpty()) {
				_desktopFile = "bettergram.desktop";
				LOG(("Found Unity Launcher entry bettergram.desktop!"));
				useUnityCount=true;
			} else if(!QStandardPaths::locate(QStandardPaths::ApplicationsLocation, "Bettergram.desktop").isEmpty()) {
				_desktopFile = "bettergram.desktop";
				LOG(("Found Unity Launcher entry Bettergram.desktop!"));
				useUnityCount=true;
			} else {
			...
			LOG(("SNAP Enviroment detected, setting Launcher entry to %1-bettergram.desktop!").arg(snapName));
			_desktopFile = snapName + "_bettergram.desktop";
			useUnityCount=true;
		}
