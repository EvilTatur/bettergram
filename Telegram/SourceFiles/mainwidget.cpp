	...
	if (Adaptive::ThreeColumn()) {
		if (!_thirdSection
			...
			auto params = Window::SectionShow(
				...
				anim::type::instant,
				anim::activation::background);
			if (session().settings().bettergramTabsSectionEnabled()) {
				_history->pushBettergramTabsToThirdSection(params);
			} else if (session().settings().tabbedSelectorSectionEnabled()) {
...
void MainWidget::updateThirdColumnToCurrentChat(
	...
	if (Adaptive::ThreeColumn()
		&& session().settings().bettergramTabsSectionEnabled()
		&& key) {
		if (!canWrite) {
			switchInfoFast();
			session().settings().setBettergramTabsSectionEnabled(true);
